...
{ 
  ...
  if(isset($kpi_btn)) { 
    ...
    $GLOBALS['$Fdata'] = [];
    $GLOBALS['kpi']= array(); 
    $kpiErr = "البيانات خاطئة للمندوب صاحب الكود: ";  
    Excel::load($upload_success, function($reader)
        {   
          ...
          {    
            ...
            if ($facebook_regex ==1 || !isset($data['facebook'])) {
              if ($instgram_regex ==1 || !isset($data['instgram'])) {
                if ($Website_regex ==1 || !isset($data['website'])) {
                  if ($youtube_regex ==1 || !isset($data['youtube'])) {
                    if ($Report_regex ==1 || !isset($data['reports'])) {
                      if ($NewCon_regex ==1 || !isset($data['newcon'])) {
                        if ($Bonus_regex ==1 || !isset($data['bonus'])) {
                          if ($Others_regex ==1 || !isset($data['others'])) {
                             if ($Bnd_regex ==1 || !isset($data['bnd'])) {
                              //push values if valid
                                $Pormoter_Id= Promoter::where('Code',$data['code'])
                                                  ->pluck('Pormoter_Id')->first();
...
$resumation = ceil(number_format($resumation, 2, '.', ''));
          ...
          array_push($Final,array('Name'=>$Name->Pormoter_Name,
                                 ...
                                 array_push($GLOBALS['kpi'],$data['code']);
                              }  
